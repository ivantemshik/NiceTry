...
- [x] **Приём платежей (Pay4game) — БОЕВОЙ режим** ✅
  - Боевой шлюз pay4game: `payment/create` (СБП qr/url, карта, sberpay), вебхуки
    `inform/status/status_steam/status_payoff/status_topup`, подпись запросов и проверка подписи
    вебхука (constant-time), Steam-пополнение одним платежом (v2 check/balance в админке), выплаты
    СБП/карта (под фичефлагом). Клиент `src/lib/payments/pay4game.ts`, абстракция
    `src/lib/payments/{index,live,mock}.ts` (переключение флагом `PAYMENTS_MODE`).
  - **Выдача по факту оплаты — реальная:** вебхук `status` (success && hold=0) идемпотентно
    переводит заказ `new→paid` и выдаёт instant-позиции через общий `deliverInstant`
    (AppRoute / Dessly / локальные ключи) — тот же модуль, что и оплата с баланса. Сбой поставщика
    не роняет вебхук: позиция остаётся `pending`, заказ — `paid` (в работе).
  - **Отложенная Dessly-выдача:** `form_data` позиции (invite-ссылка Steam, регион, издание)
    сохраняется на чекауте в `order_items.form_data` и читается вебхуком при выдаче гифта
    (миграция `migrations/2026-06-08_order_items_form_data.sql`).
  - **БД:** `migrations/2026-06-08_pay4game.sql` (таблицы `payments`, `payment_webhooks`).
  - **Статус:** код боевой, тесты зелёные (подпись/идемпотентность/hold/выдача/Steam).
    Боевой режим включается `PAYMENTS_MODE=live` + ключами pay4game (см. «Нужно от владельца»).

- [x] **Гостевой чекаут + ник после оплаты** ✅
  - Гость указывает почту → оплачивает (pay4game СБП/карта) → после успешной оплаты экран
    «придумайте ник» → автологин в личный кабинет. Завязка на pay4game снята: внешний приём денег
    не требует предзаведённого баланса.
  - **Поток:** `POST /api/checkout/guest` (live: заказ `new`, позиции `pending`, создаёт платёж) →
    страница оплаты `/pay/[invoiceId]` (поллинг статуса, QR/iframe/ссылка) → `finalize` (ник →
    аккаунт → авто-вход через checkout-токен). В mock-режиме «оплата» синхронная (DEMO-коды).
...
5. **Платёжная система (Pay4game) — код боевой, нужно включить:**
   - Применить `migrations/2026-06-08_pay4game.sql` и `migrations/2026-06-08_order_items_form_data.sql`
     на боевой Supabase (последняя — для отложенной Dessly-выдачи через `form_data`).
   - Заполнить env (Vercel): `PAY4GAME_API_TOKEN`, `PAY4GAME_SECRET_KEY`, `PAY4GAME_PROJECT_ID`
     (опц. `PAY4GAME_DEFAULT_METHOD`, `PAY4GAME_SBP_TYPE`, `PAY4GAME_RETURN_URL`, `PAY4GAME_PAYOUTS_ENABLED`).
   - В панели pay4game «Настройки»: webhook = `https://www.nicetry.guru/api/pay4game/webhook`,
     страница результата = `https://www.nicetry.guru/pay/return/#invoice_id#`.
   - Выставить `PAYMENTS_MODE=live` на Vercel (сейчас `mock` — безопасно).
